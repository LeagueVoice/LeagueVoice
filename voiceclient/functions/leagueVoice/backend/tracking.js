...
const getUser = function(uniqueID, my_firebase) {
	return new Promise((resolve, reject) => {
   	my_firebase.database()
		  .ref('users')
		  .once('value', snapshot => resolve(snapshot.val()[uniqueID]), reject);
	});
};
...
const getWinrateForChamp = function(uniqueID, championID) {
	...
	ref.once('value')
		.then(snap => snap.val().win / snap.val().total * 100);
};
...
const getUsersMostPlayedLane = function(uniqueID, summonerID, region) {
	...
	return ref.child("match").once('value')
	 	.then(snap => {
			return snap.map(item => {
				...
				const lanePlayed = { "top" : top, "mid" : mid, "jung" : jung, "bot" : bot };
				let mostPlayedLane = Object.keys(lanePlayed)
					.reduce((a, b) => lanePlayed[a] > lanePlayed[b] ? a : b);
