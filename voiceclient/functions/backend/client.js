...
// from the Riot API.
function getBySummonerName(name, region) {
  const options = {
    method: 'GET',
...
function getBySummonerId(summonerID, region) {
  const options = {
    method: 'GET',
...
function getRecentMatchList(accountID, region) {
  const options = {
    method: 'GET',
...
// Returns a promise for getRecentMatchList that resolves to the returned JSON
// from the Riot API matchlists/by-account/./recent endpoint.
function getMatchList(accountID, region) {
  const options = {
    method: 'GET',
...
function getCurrentMatch(summonerID, region = "NA1") {
  const options = {
    method: 'GET',
...
function getAllLeaguePositionsForSummoner(summonerID, region) {
  const options = {
    method: 'GET',
...
function getMatchlistForQueue(accountID, region, champion, queue = 420) {
  const options = {
    method: 'GET',
...
function getAllChampionMasteries(summonerID, region) {
  const options = {
    method: 'GET',
...
function getAllChampionMasteriesForSummoner(summonerID, region) {
	const options = {
		method: 'GET',
		uri: `${APIPROXY}/rgapi/${region}/championMastery/getAllChampionMasteries/${summonerID}`,
		json: true
 ...
 */
function getGGChampionsForRole (position, rank = 'PLATINUM') {
  const options = {
    method: 'GET',
  ...
  return rp(options)
    .then(champions => champions.filter(champion => champion.role === position));
}

// Returns an approximate "Sharpe" on winrate versus 50%.
function rateSignificance(winrate, n) {
...
function getBestMatchupsByLane(championID, rank = 'BRONZE') {
  const options = {
    method: 'GET',
  ...
  return rp(options).then(function(matchups) {
    ...
    let byRole = roles.map(function(role) {
      ...
      let counts = forRole.map(elem => elem.count);
...
module.exports = {
  ...
  getAllChampionMasteries,
	...
	getAllChampionMasteriesForSummoner,
  getGGChampionsForRole,
  getBestMatchupsByLane,
  getChampionName,
  getChampionID,
  getCurrentMatch
};
